...
if (!function_exists('rdirOut')) {
    function rdirOut($sections, $section_key, $field_key, $default = '') {
...
if (!function_exists('rdirIcon')) {
    function rdirIcon($sections, $section_key, $field_key, $default = 'star') {
        $icon = preg_replace('/[^a-z0-9_]/', '', strtolower(getContent($sections, $section_key, $field_key, $default)));
        return $icon !== '' ? $icon : $default;
    }
}

$page_title = $page ? $page['page_title'] . " - Valley View University" : "Office of Research, Development & International Relations - Valley View University";
$active_page = "about";
...
<style>
    ...
    .rdir-card { transition: transform .35s ease, box-shadow .35s ease, border-color .35s ease; }
    .rdir-card:hover { transform: translateY(-6px); box-shadow: 0 26px 50px -22px rgba(15, 23, 42, .40); }

    /* Sticky portrait rail on large screens only */
    @media (min-width: 1024px) {
        .rdir-sticky { position: sticky; top: 110px; }
    }

    /* Prose flows as one continuous column at every width. */
    .rdir-bio p + p,
    .rdir-prose p + p { margin-top: 1.5rem; }
</style>
...
<main class="flex-grow bg-gray-50 dark:bg-gray-900">
            ...
            <div class="rdir-wrap">
                <div class="flex flex-col lg:flex-row gap-12 lg:gap-16 items-center lg:items-start">
                    ...
                    <div class="w-full max-w-sm lg:max-w-none lg:w-1/4 animate-fadeInUp">
                        <div class="rdir-sticky relative group">
                            ...
                            <div class="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-2xl border-4 border-white dark:border-gray-800">
                                <img src="<?php echo strip_tags(getContent($pageContent, 'officer_profile', 'profile_image', 'images/leadership/prof-josephine-ganu.png')); ?>"
                                     alt="<?php echo rdirOut($pageContent, 'officer_profile', 'name', 'Dean of RDIR'); ?>"
                                     class="w-full h-full object-cover object-top">
                            </div>
                            <div class="mt-6 text-center lg:text-left">
                                <h2 class="text-3xl sm:text-4xl font-black text-gray-900 dark:text-white mb-1 tracking-tight"><?php echo rdirOut($pageContent, 'officer_profile', 'name', 'Dean of RDIR'); ?></h2>
                                <p class="text-base font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest"><?php echo rdirOut($pageContent, 'officer_profile', 'title', 'Dean, RDIR'); ?></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full lg:w-3/4 space-y-10 animate-fadeInUp" style="animation-delay: 0.2s;">
                        <div>
                            <span class="inline-block px-5 py-2 mb-6 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-base font-black uppercase tracking-widest">
                                <?php echo rdirOut($pageContent, 'officer_profile', 'section_label', 'Meet the Dean'); ?>
                            </span>
                            <h3 class="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white mb-5 tracking-tight"><?php echo rdirOut($pageContent, 'officer_profile', 'section_title', 'Profile & Biography'); ?></h3>
                            <div class="h-2 w-32 bg-yellow-400 rounded-full mb-8"></div>
                            <div class="rdir-bio text-lg sm:text-xl text-gray-600 dark:text-gray-400 leading-relaxed">
                                <?php for ($i = 1; $i <= 5; $i++):
        ...
        <div class="container">
            <div class="max-w-5xl mx-auto text-center mb-16">
                <span class="inline-block px-5 py-2 mb-6 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 text-base font-black uppercase tracking-widest">
                    <?php echo rdirOut($pageContent, 'leadership_roles', 'section_label', 'Track Record'); ?>
                </span>
                <h2 class="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white mb-5 tracking-tight">
                    <?php echo rdirOut($pageContent, 'leadership_roles', 'section_title', 'Leadership & Service'); ?>
                </h2>
                ...
                <p class="text-lg sm:text-xl text-gray-600 dark:text-gray-400 leading-relaxed">
                    <?php echo rdirOut($pageContent, 'leadership_roles', 'section_description', ''); ?>
                </p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 rdir-wrap">
                <?php for ($n = 1; $n <= 6; $n++):
                    $rtitle = getContent($pageContent, 'leadership_roles', "role_{$n}_title");
                    if ($rtitle === '') continue; ?>
                <div class="rdir-card group p-10 bg-white dark:bg-gray-900 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800">
                    <div class="flex items-center justify-between mb-6">
                        <div class="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors">
                            <span class="material-symbols-outlined text-4xl"><?php echo rdirIcon($pageContent, 'leadership_roles', "role_{$n}_icon"); ?></span>
                        </div>
                    ...
                    <p class="text-lg text-gray-600 dark:text-gray-400 leading-relaxed">
                        <?php echo rdirOut($pageContent, 'leadership_roles', "role_{$n}_text", ''); ?>
                    </p>
                </div>
                <?php endfor; ?>
            </div>
        ...
        <div class="container">
            <div class="max-w-5xl mx-auto text-center mb-16">
                <span class="inline-block px-5 py-2 mb-6 rounded-full bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-base font-black uppercase tracking-widest">
                    <?php echo rdirOut($pageContent, 'credentials', 'section_label', 'Qualifications'); ?>
                </span>
                <h2 class="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white mb-5 tracking-tight">
                    <?php echo rdirOut($pageContent, 'credentials', 'section_title', 'Academic & Professional Standing'); ?>
                </h2>
                <div class="h-2 w-36 bg-green-600 mx-auto rounded-full mb-7"></div>
                <p class="text-lg sm:text-xl text-gray-600 dark:text-gray-400 leading-relaxed">
                    <?php echo rdirOut($pageContent, 'credentials', 'section_description', ''); ?>
                </p>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 rdir-wrap mb-12">
                <!-- Academic -->
                <div class="p-10 sm:p-12 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-lg">
                    <h3 class="flex items-center gap-4 text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mb-8">
                        ...
                        <?php echo rdirOut($pageContent, 'credentials', 'academic_title', 'Academic Qualifications'); ?>
                    </h3>
                    <ul class="space-y-6">
                        <?php for ($n = 1; $n <= 4; $n++):
                        ...
                        <li class="flex gap-5">
                            ...
                            <div>
                                ...
                                <p class="text-lg text-gray-600 dark:text-gray-400"><?php echo rdirOut($pageContent, 'credentials', "academic_{$n}_text", ''); ?></p>
                            </div>
                        </li>
                        <?php endfor; ?>
                    </ul>
                ...
                <div class="p-10 sm:p-12 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-lg">
                    <h3 class="flex items-center gap-4 text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mb-8">
                        ...
                        <?php echo rdirOut($pageContent, 'credentials', 'memberships_title', 'Professional Memberships'); ?>
                    </h3>
                    <ul class="space-y-5">
                        <?php for ($n = 1; $n <= 4; $n++):
                            $m = getContent($pageContent, 'credentials', "membership_{$n}");
                            if ($m === '') continue; ?>
                        <li class="flex items-start gap-4">
                            <span class="material-symbols-outlined text-3xl text-green-600 shrink-0">check_circle</span>
                            <span class="text-lg sm:text-xl font-bold text-gray-800 dark:text-gray-200 leading-snug"><?php echo htmlspecialchars($m, ENT_QUOTES, 'UTF-8'); ?></span>
                        </li>
                        <?php endfor; ?>
                    </ul>
                </div>
            ...
            <!-- Research interests -->
            <div class="rdir-wrap">
                <h3 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mb-8 text-center"><?php echo rdirOut($pageContent, 'credentials', 'interests_title', 'Research Interests'); ?></h3>
                <div class="grid grid-cols-2 lg:grid-cols-5 gap-6">
                    <?php for ($n = 1; $n <= 5; $n++):
                        $ititle = getContent($pageContent, 'credentials', "interest_{$n}_title");
                        if ($ititle === '') continue; ?>
                    <div class="rdir-card group p-8 bg-gray-50 dark:bg-gray-800/50 rounded-3xl text-center shadow-lg border border-gray-100 dark:border-gray-800">
                        <div class="w-20 h-20 mx-auto rounded-2xl bg-white dark:bg-gray-900 text-purple-600 dark:text-purple-400 flex items-center justify-center shadow mb-5 group-hover:bg-purple-600 group-hover:text-white transition-colors">
                            <span class="material-symbols-outlined text-5xl"><?php echo rdirIcon($pageContent, 'credentials', "interest_{$n}_icon"); ?></span>
                        </div>
                        <h4 class="text-xl font-black text-gray-900 dark:text-white leading-tight"><?php echo htmlspecialchars($ititle, ENT_QUOTES, 'UTF-8'); ?></h4>
                    </div>
                    <?php endfor; ?>
                </div>
            </div>
        ...
        <div class="container">
            <div class="rdir-wrap">
                <div class="max-w-5xl mx-auto text-center mb-14">
                    <span class="inline-block px-5 py-2 mb-6 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-base font-black uppercase tracking-widest">
                        <?php echo rdirOut($pageContent, 'office_overview', 'section_label', 'About RDIR'); ?>
                    </span>
                    <h2 class="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white mb-5 tracking-tight">
                        <?php echo rdirOut($pageContent, 'office_overview', 'section_title', 'About the Office'); ?>
                    </h2>
                    <div class="h-2 w-36 bg-yellow-400 mx-auto rounded-full"></div>
                </div>

                <div class="rdir-prose text-lg sm:text-xl text-gray-600 dark:text-gray-400 leading-relaxed max-w-5xl mx-auto">
                    <?php for ($i = 1; $i <= 3; $i++):
        ...
        <div class="container">
            <div class="max-w-5xl mx-auto text-center mb-16">
                <span class="inline-block px-5 py-2 mb-6 rounded-full bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-base font-black uppercase tracking-widest">
                    <?php echo rdirOut($pageContent, 'focus_areas', 'section_label', 'What We Do'); ?>
                </span>
                <h2 class="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white mb-5 tracking-tight">
                    <?php echo rdirOut($pageContent, 'focus_areas', 'section_title', 'Strategic Focus Areas'); ?>
                </h2>
                <div class="h-2 w-36 bg-purple-600 mx-auto rounded-full mb-7"></div>
                <p class="text-lg sm:text-xl text-gray-600 dark:text-gray-400 leading-relaxed">
                    <?php echo rdirOut($pageContent, 'focus_areas', 'section_description', ''); ?>
                </p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 rdir-wrap">
                <?php for ($n = 1; $n <= 6; $n++):
                    $atitle = getContent($pageContent, 'focus_areas', "area_{$n}_title");
                    if ($atitle === '') continue; ?>
                <div class="rdir-card group p-10 bg-gray-50 dark:bg-gray-800/50 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800">
                    <div class="w-16 h-16 rounded-2xl bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 flex items-center justify-center mb-5 group-hover:bg-purple-600 group-hover:text-white transition-colors">
                        <span class="material-symbols-outlined text-4xl"><?php echo rdirIcon($pageContent, 'focus_areas', "area_{$n}_icon"); ?></span>
                    </div>
                    <h4 class="text-2xl font-black text-gray-900 dark:text-white mb-3 leading-tight"><?php echo htmlspecialchars($atitle, ENT_QUOTES, 'UTF-8'); ?></h4>
                    <p class="text-lg text-gray-600 dark:text-gray-400 leading-relaxed">
                        <?php echo rdirOut($pageContent, 'focus_areas', "area_{$n}_text", ''); ?>
                    </p>
                </div>
                <?php endfor; ?>
            </div>
        ...
        <div class="container">
            <div class="max-w-5xl mx-auto text-center mb-16">
                <span class="inline-block px-5 py-2 mb-6 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-base font-black uppercase tracking-widest">
                    <?php echo rdirOut($pageContent, 'related_offices', 'section_label', 'Explore Further'); ?>
                </span>
                <h2 class="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white mb-5 tracking-tight">
                    <?php echo rdirOut($pageContent, 'related_offices', 'section_title', 'Related Offices & Resources'); ?>
                </h2>
                <div class="h-2 w-32 bg-blue-600 mx-auto rounded-full"></div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 rdir-wrap">
                <?php for ($n = 1; $n <= 4; $n++):
                    $ltitle = getContent($pageContent, 'related_offices', "link_{$n}_title");
                    if ($ltitle === '') continue; ?>
                <a href="<?php echo rdirOut($pageContent, 'related_offices', "link_{$n}_url", '#'); ?>"
                   class="rdir-card group block p-9 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 hover:border-blue-500 dark:hover:border-blue-500 no-underline shadow-lg">
                    <div class="w-14 h-14 rounded-xl bg-gray-50 dark:bg-gray-800 text-blue-600 dark:text-blue-400 flex items-center justify-center shadow mb-5 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                        <span class="material-symbols-outlined text-3xl"><?php echo rdirIcon($pageContent, 'related_offices', "link_{$n}_icon", 'link'); ?></span>
                    </div>
                    ...
                    <p class="text-base text-gray-600 dark:text-gray-400 leading-relaxed mb-4"><?php echo rdirOut($pageContent, 'related_offices', "link_{$n}_text", ''); ?></p>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
                        ...
                        <div>
                            <span class="inline-block px-4 py-1.5 mb-5 rounded-full bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-sm font-black uppercase tracking-widest">
                                <?php echo rdirOut($pageContent, 'office_contact', 'section_label', 'Get in Touch'); ?>
                            </span>
                            <h3 class="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white mb-4 tracking-tight"><?php echo rdirOut($pageContent, 'office_contact', 'section_title', 'Contact the Office'); ?></h3>
                            <p class="text-lg sm:text-xl text-gray-600 dark:text-gray-400 mb-9 leading-relaxed">
                                <?php echo rdirOut($pageContent, 'office_contact', 'section_description', ''); ?>
                            </p>
                        ...
                        <div class="bg-white dark:bg-gray-800/50 p-10 sm:p-12 rounded-[2rem] border border-gray-100 dark:border-gray-700">
                            <h4 class="text-3xl font-black text-gray-900 dark:text-white mb-3 tracking-tight"><?php echo rdirOut($pageContent, 'office_contact', 'form_title', 'Research & Partnership Inquiry'); ?></h4>
                            <p class="text-lg text-gray-600 dark:text-gray-400 mb-7 leading-relaxed">
                                <?php echo rdirOut($pageContent, 'office_contact', 'form_description', ''); ?>
                            </p>
                            <form action="contact_process.php" method="post" class="space-y-4">
                                <input type="hidden" name="inquiry-type" value="Research &amp; Partnership Inquiry - Office of RDIR">
                                <div>
                                    <label for="rdir_name" class="sr-only">Full name</label>
                                    <input id="rdir_name" name="name" type="text" required placeholder="Your Full Name"
                                ...
                                <div>
                                    <label for="rdir_email" class="sr-only">Email address</label>
                                    <input id="rdir_email" name="email" type="email" required placeholder="Email Address"
                                           class="w-full px-6 py-5 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600 outline-none text-lg transition">
                                </div>
                                <div>
                                    <label for="rdir_msg" class="sr-only">Your inquiry</label>
                                    <textarea id="rdir_msg" name="message" rows="5" required placeholder="Your Inquiry"
                                ...
                                <button type="submit" class="w-full py-5 bg-blue-600 hover:bg-blue-700 text-white text-xl font-black rounded-2xl transition-all shadow-lg hover:shadow-blue-500/25 flex items-center justify-center gap-2">
                                    ...
                                    <?php echo rdirOut($pageContent, 'office_contact', 'form_btn_text', 'Submit Inquiry'); ?>
                                </button>
                            </form>
                        </div>
                    </div>
    ...
    <section class="relative py-24 sm:py-32 overflow-hidden">
        ...
        <div class="container relative z-10">
            <div class="rdir-wrap text-center">
                <h2 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6 leading-tight tracking-tight">
                    <?php echo rdirOut($pageContent, 'cta_section', 'cta_title', 'Research Excellence &'); ?>
                    <span class="text-yellow-400 block mt-2"><?php echo rdirOut($pageContent, 'cta_section', 'cta_highlight', 'Global Impact'); ?></span>
                </h2>
                <p class="text-lg sm:text-xl lg:text-2xl text-blue-100 mb-10 max-w-4xl mx-auto leading-relaxed">
                    <?php echo rdirOut($pageContent, 'cta_section', 'cta_description', ''); ?>
                </p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="<?php echo rdirOut($pageContent, 'cta_section', 'button_1_url', 'research_opportunities.php'); ?>" class="px-10 py-5 bg-yellow-400 hover:bg-yellow-300 text-blue-900 text-lg font-black rounded-2xl transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-3">
                        <span class="material-symbols-outlined text-3xl">science</span>
                        <?php echo rdirOut($pageContent, 'cta_section', 'button_1_text', 'Research Opportunities'); ?>
                    </a>
                    <a href="<?php echo rdirOut($pageContent, 'cta_section', 'button_2_url', 'contact_us.php'); ?>" class="px-10 py-5 bg-white/10 hover:bg-white/20 text-white text-lg font-black rounded-2xl transition-all backdrop-blur-md border-2 border-white/30 transform hover:scale-105 shadow-lg flex items-center justify-center gap-3">
                        <span class="material-symbols-outlined text-3xl">handshake</span>
                        <?php echo rdirOut($pageContent, 'cta_section', 'button_2_text', 'Partner With Us'); ?>
                    </a>
                </div>
            </div>
        </div>
