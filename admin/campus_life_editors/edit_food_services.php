...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_food'])) {
    ...
    if (isset($_FILES['philosophy_image_upload']) && $_FILES['philosophy_image_upload']['error'] === UPLOAD_ERR_OK) {
        $filename = uniqid('philosophy_') . '_' . basename($_FILES['philosophy_image_upload']['name']);
        if (move_uploaded_file($_FILES['philosophy_image_upload']['tmp_name'], $upload_dir . $filename)) {
            $philosophy_image = 'uploads/campus_life/' . $filename;
        }
    }
