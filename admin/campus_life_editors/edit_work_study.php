...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_work_study'])) {
    ...
    if (isset($_FILES['overview_image_upload']) && $_FILES['overview_image_upload']['error'] === UPLOAD_ERR_OK) {
        $filename = uniqid('overview_') . '_' . basename($_FILES['overview_image_upload']['name']);
        if (move_uploaded_file($_FILES['overview_image_upload']['tmp_name'], $upload_dir . $filename)) {
            $overview_image = 'uploads/campus_life/' . $filename;
        }
    }
